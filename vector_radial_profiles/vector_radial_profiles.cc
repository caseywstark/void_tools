...
main(int argc, char **argv)
{
    ...
    const std::string fx_path = argv[2];
    const std::string fy_path = argv[3];
    const std::string fz_path = argv[4];
    const std::string pre_path = argv[5];
    ...
    double *fx = new double[np_local];
    double *fy = new double[np_local];
    double *fz = new double[np_local];

    FILE *field_file;

    field_file = fopen(fx_path.c_str(), "r");
    fseek(field_file, sizeof(double) * local_ix0 * n * n, SEEK_SET);
    fread(fx, sizeof(double), np_local, field_file);
    fclose(field_file);

    field_file = fopen(fy_path.c_str(), "r");
    fseek(field_file, sizeof(double) * local_ix0 * n * n, SEEK_SET);
    fread(fy, sizeof(double), np_local, field_file);
    fclose(field_file);

    field_file = fopen(fz_path.c_str(), "r");
    fseek(field_file, sizeof(double) * local_ix0 * n * n, SEEK_SET);
    fread(fz, sizeof(double), np_local, field_file);
    fclose(field_file);
            ...
            if (ix_local >= 0 && ix_local < n_local) {
                ...
                for (long iy = iy0; iy <= iy1; ++iy) {
                    ...
                    for (long iz = iz0; iz <= iz1; ++iz) {
                        ...
                        if (ibin < num_bins) {
                            // field values
                            size_t ii = (ix_local * n + iyw) * n + izw;
                            double fxi = fx[ii];
                            double fyi = fy[ii];
                            double fzi = fz[ii];

                            // the radial component.
                            double fr = (fxi * rx + fyi * ry + fzi * rz) / r;

                            // accumulate
                            bin_counts_local[ibin] += 1;
                            bin_r_sums_local[ibin] += r;
                            bin_f_sums_local[ibin] += fr;
                        }
                    }
                }
            }
